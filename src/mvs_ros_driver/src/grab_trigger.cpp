...
void CameraTimeSyncCallback(const std_msgs::Time& msg) {
  timestamp_mutex.lock();
  frame_timestamp = msg.data;
...
static void *WorkThread(void *pUser) {
  ...
  while (!exit_flag && ros::ok()) {
    ...
    if (nRet == MV_OK) {
      ...
      /*ros::Time rcv_time = frame_timestamp;
      ...
      timestamp_mutex.lock();
      msg->header.stamp = frame_timestamp;
      timestamp_mutex.unlock();
      pub.publish(msg);
    }
  }
...
int main(int argc, char **argv) {
  ...
  ROS_INFO("Starting time sync subscriber...");
  ros::Subscriber timesyncSubscriber = nh.subscribe("/wit/camera", 1, CameraTimeSyncCallback);
